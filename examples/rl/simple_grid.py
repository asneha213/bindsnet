...
class SimpleGrid:
    ...
    def transition_function(self, state, action):
        ...
        reward = None
        if action == "U":
            if state <= 2 or state == 8 or state == 7:
                new_state = state
            else:
                new_state = state - 3
            ...
            if state == 0 or state == 3 or state == 6 or state == 8 \
                    or state == 5:
                new_state = state
            else:
                new_state = state - 1
...
class SarsaGW():
    def __init__(self, epsilon, alpha, lda):
        ...
        self.sigma = 10
        self.q = 10*np.ones((9, 4)) + np.random.normal(0,1, size=(9,4))
        self.gw = SimpleGrid()
    ...
    def run_actor_critic(self, num_episodes):
        returns = []
        theta = np.random.rand(9, 4)
        v_f = np.random.rand(9)
        alpha = 0.5
        beta = 0.5
        for i in range(num_episodes):
            rewards = []
